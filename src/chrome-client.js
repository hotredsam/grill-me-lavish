...
const queueStorageKey = "grill-me-lavish:queued:" + key;
...
function updateSendState() {
  // grill-me-lavish: never block sends while the agent is working - feedback
  // queues server-side and is delivered on the agent's next poll.
...
function setLayoutGateCard(state) {
  ...
  layoutGateCopy.textContent = "grill-me-lavish is waiting for fonts and final geometry before revealing this artifact.";
}
...
function forceRevealLayoutGate(reason) {
  ...
  const bannerText =
    reason === "timeout"
      ? "This surface may have layout issues. grill-me-lavish revealed it after the safety timeout so review is never blocked."
