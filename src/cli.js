...
const DESCRIPTION =
  ...
  "First generate an interactive HTML artifact according to user request, then run `grill-me-lavish <html-file>` so the user can visually review it, annotate elements or selected text, queue prompts, and send feedback back through `grill-me-lavish poll`.";
...
export async function run(argv) {
  ...
  try {
    await runAxiCli({
      ...
      home: async () =>
        createHomeOutput({
          bin: process.argv[1] || "grill-me-lavish",
...
export function createPlaybookOutput(args) {
  ...
  if (!id) {
    return {
      playbooks: listPlaybooks(),
      help: ["Run `grill-me-lavish playbook <playbook_id>` for focused artifact guidance", PLAYBOOK_ROUTER_HELP],
    };
  }
  ...
  if (!playbook) {
    throw new AxiError(`Unknown playbook: ${id}`, "VALIDATION_ERROR", [
      `Run \`grill-me-lavish playbook\` to list known IDs: ${playbookIds().join(", ")}`,
    ]);
  }
...
export function createUserEndedOpenOutput({ file, url }) {
  return {
    ...
    next_step: `The user explicitly ended this Lavish Editor session from the browser, so \`grill-me-lavish ${file}\` did not reopen it. Do not reopen unless the user asks for further review or something genuinely important needs their visual attention - deliver routine updates directly in this conversation instead. When reopening is warranted, run \`grill-me-lavish ${file} --reopen\`.`,
  };
}
...
async function openCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `grill-me-lavish <html-file>`"]);
  }
...
async function pollCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `grill-me-lavish poll <html-file>`"]);
  }
  const absolute = await canonicalFile(file);
...
export function pollWaitBannerText(file) {
  return (
    `[grill-me-lavish] Long-polling for user feedback or layout_warnings on ${file}. This stays silent until the user sends feedback, ends the session, or the browser reports fresh layout_warnings - leave it running. ` +
    `If it gets killed or times out, re-run \`grill-me-lavish poll ${file}\` - queued feedback is never lost.`
  );
}

export function pollWaitTickText(elapsedMs) {
  const minutes = Math.round(elapsedMs / 60_000);
  return `[grill-me-lavish] Still waiting for user feedback (${minutes}m). Also waiting for fresh layout_warnings. Leave this running until the user acts or the browser reports fresh layout_warnings.`;
}

export function pollInterruptedText(file) {
  return (
    `[grill-me-lavish] Poll interrupted before user feedback arrived. The user may still be reviewing - ` +
    `re-run \`grill-me-lavish poll ${file}\` to keep waiting; queued feedback is never lost.`
  );
}
...
export function createPollOutput({ file, response }) {
  if (response.status === "missing") {
    throw new AxiError("No active Lavish Editor session for this file", "NOT_FOUND", [
      `Run \`grill-me-lavish ${file}\` first`,
    ]);
  }
  ...
  return {
    ...
    next_step: `No user feedback arrived before the optional timeout. Run \`grill-me-lavish poll ${file}\` without --timeout-ms to wait indefinitely - queued feedback is never lost, so re-running the poll is always safe.`,
  };
}
...
function createFeedbackNextStep(file, layoutWarnings, sessionEnded, endedBy) {
  ...
  if (sessionEnded) {
    ...
    if (endedBy === "user") {
      return `${layoutNote}This was the last feedback before the user ended the session. Stop polling ${file} and do not reopen it - deliver any remaining updates directly in this conversation instead. Only run \`grill-me-lavish ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
    }
    return `${layoutNote}This was the last feedback before the Lavish Editor session ended. Stop polling ${file}. Deliver any remaining updates directly in this conversation, or run \`grill-me-lavish ${file}\` to open a fresh session if the user needs further visual review.`;
  }
  const layoutPrefix =
    count > 0 ? layoutWarningsPrefix(file, layoutWarnings) : `Apply the requested changes to ${file}. `;
  return `${layoutPrefix}Do not respond to the user just yet. Now you must run \`grill-me-lavish poll ${file} --agent-reply "<message for the user>"\` without --timeout-ms unless the user ended the session. The poll waits silently until the user sends more feedback, ends the session, or reports fresh layout_warnings - never kill it. If your harness limits how long a foreground command may run, run the poll as a background task; if it still gets killed or times out, just re-run it - queued feedback is never lost.`;
}
...
function layoutWarningsPrefix(file, layoutWarnings) {
  ...
  return `${count} layout warning${plural} detected - fix horizontal overflow or clipped text in ${file}, then re-check in the browser before involving the human. Lavish live-reloads the artifact automatically after you save, so you do not need to re-run \`grill-me-lavish ${file}\` for this. `;
}

function createEndedNextStep(file, endedBy) {
  if (endedBy === "user") {
    return `The user ended this Lavish Editor session. Stop polling ${file} - do not run \`grill-me-lavish ${file}\` to reopen it. Deliver any remaining updates directly in this conversation instead. Only reopen with \`grill-me-lavish ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
  }
  return `This Lavish Editor session for ${file} has ended. Stop polling. Deliver any remaining updates directly in this conversation, or run \`grill-me-lavish ${file}\` to open a fresh session if the user needs further visual review.`;
}

async function endCommand(args) {
  const file = firstPositionalArg(args);
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `grill-me-lavish end <html-file>`"]);
  }
  const absolute = await canonicalFile(file);
  const baseUrl = await ensureServer();
...
async function exportCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `grill-me-lavish export <html-file>`"]);
  }
  await assertHtmlFile(file);
  const absolute = await canonicalFile(file);
...
async function shareCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `grill-me-lavish share <html-file>`"]);
  }
  await assertHtmlFile(file);
  const absolute = await canonicalFile(file);
...
async function setupCommand(args) {
  if (args.length !== 1 || args[0] !== "hooks") {
    throw new AxiError("Unknown setup action", "VALIDATION_ERROR", ["Run `grill-me-lavish setup hooks`"]);
  }

  const errors = [];
  installSessionStartHooks({
    marker: "grill-me-lavish",
    binaryNames: ["grill-me-lavish"],
    distEntrypoints: ["dist/cli.mjs", "bin/grill-me-lavish.js"],
  ...
  if (errors.length > 0) {
    throw new AxiError("Failed to install grill-me-lavish agent hooks", "SERVER_ERROR", errors);
  }

  return {
    hooks: { status: "installed", integrations: "Claude Code, Codex, OpenCode, GitHub Copilot CLI" },
    help: ["Restart your agent session to receive grill-me-lavish ambient context"],
  };
}
...
export function createCopilotCliAmbientContextScript(command = "grill-me-lavish") {
  return [
    ...
    'const prefix = result.status === 0 ? "## AXI ambient context: grill-me-lavish\\n" : "## AXI ambient context: grill-me-lavish\\nerror: grill-me-lavish ambient context failed: ";',
    "console.log(JSON.stringify({ additionalContext: prefix + text }));",
  ].join(" ");
}

export function createCopilotCliSessionStartHook(command = "grill-me-lavish", timeoutSec = 10) {
...
export function installCopilotCliSessionStartHook({
  ...
  command = "grill-me-lavish",
  timeoutSec = 10,
  onError = undefined,
} = {}) {
  const target = path.join(hookDir, "grill-me-lavish.json");
  try {
...
function isManagedCopilotCliHook(entry) {
  return (
    ...
    [entry.bash, entry.powershell, entry.command].some(
      (value) => typeof value === "string" && value.includes("grill-me-lavish"),
    )
  );
}
...
async function assertHtmlFile(file) {
  if (!isHtmlPath(file)) {
    throw new AxiError("Lavish Editor expects an HTML file", "VALIDATION_ERROR", ["Run `grill-me-lavish <html-file>`"]);
  }
  try {
    await access(file);
  } catch {
    throw new AxiError(`File not found: ${file}`, "NOT_FOUND", [
      "Create the HTML artifact first, then run `grill-me-lavish <html-file>`",
    ]);
  }
}
...
async function ensureServer({ forceRestart = false } = {}) {
  ...
  throw new AxiError("Lavish Editor server did not start", "SERVER_ERROR", [
    `Run \`grill-me-lavish server --port ${port}\` to inspect server startup`,
  ]);
}
...
export function shouldRestartServer(currentVersion, healthBody, forceRestart = false) {
  ...
  if (forceRestart && healthBody.app === "grill-me-lavish") return true;
...
export function shouldKillProcessOnPort(currentVersion, healthBody) {
  if (!healthBody || typeof healthBody !== "object") return false;
  if (typeof healthBody.version !== "string" || healthBody.version === "") return true;
  if (healthBody.app !== "grill-me-lavish") return false;
  return healthBody.version !== currentVersion;
}

async function canControlServerOnPort(port, healthBody, processMatchesLavish) {
  if (!healthBody || typeof healthBody !== "object") return false;
  if (healthBody.app === "grill-me-lavish") return true;
...
// macOS/Linux only - Windows users would need to kill manually, but grill-me-lavish isn't
...
function processOnPortMatchesLavish(port) {
  try {
    ...
    for (const line of pids.stdout.split("\n")) {
      ...
      if (command.status === 0 && /grill-me-lavish/.test(command.stdout)) {
        return true;
      }
    }
  } catch {
...
// run(). In source layout that's `../bin/grill-me-lavish.js` (which calls run on import). In the
...
export function resolveServerEntry() {
  const binEntry = fileURLToPath(new URL("../bin/grill-me-lavish.js", import.meta.url));
...
function serverConnectionError() {
  return new AxiError("Lavish Editor server connection failed", "SERVER_ERROR", [
    "Run `grill-me-lavish server --verbose` or inspect `~/.grill-me-lavish-state/server.log` (`LAVISH_AXI_STATE_DIR/server.log` when set) for server startup or crash diagnostics",
    "Re-run the last `grill-me-lavish poll <html-file>` command after the server is healthy",
  ]);
}
