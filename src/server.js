...
// `grill-me-lavish <file>` invocation re-spawns a fresh server and adopts resumable sessions from
...
export async function serve({
  ...
  app.get("/health", (req, res) => {
    res.json({ ok: true, app: "grill-me-lavish", version });
  });
  ...
  app.post("/api/sessions", async (req, res, next) => {
    try {
      ...
      // `grill-me-lavish <file>` is the exact behavior this route exists to prevent - require an
      // explicit `reopen` opt-in instead of reviving it automatically. Agent-initiated ends
      // (`grill-me-lavish end`) keep reviving on the next open, same as before this change.
    ...
    try {
      ...
      res.type("html").send(
        createChromeHtml(session, {
          ...
          faviconTag,
          title: title ? `${title} · grill-me-lavish` : "grill-me-lavish Editor",
        }),
      );
    } catch (error) {
      next(error);
  ...
  try {
    ...
    if (hasLiveReloadRootOptIn(html)) {
      return {
        ...
        options: {
          ...baseOptions,
          ignored: /(^|[/\\])(\.git|node_modules|dist|build|\.grill-me-lavish-state)([/\\]|$)/,
        },
      };
    }
  } catch {
...
export function createChromeHtml(
  session,
  { layoutGateEnabled = true, faviconTag = LAVISH_DEFAULT_FAVICON, title = "grill-me-lavish Editor" } = {},
) {
