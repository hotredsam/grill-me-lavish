...
export const SKILL_DESCRIPTION =
  ...
  "annotate and send feedback on, using the grill-me-lavish CLI. Use when about to give a plan, " +
...
function skillCommandText(text) {
  return text.replaceAll("`grill-me-lavish", "`npx -y grill-me-lavish");
}

/**
 * Render the installable SKILL.md for the lavish skill. The body mirrors what
 * `grill-me-lavish` prints with no arguments (minus live session state), while the
...
export function createSkillMarkdown() {
  const home = createHomeOutput({ bin: "grill-me-lavish", sessions: [], includeSessions: false });
...
You do not need grill-me-lavish installed globally - invoke it with \`npx -y grill-me-lavish <html-file>\`.
If grill-me-lavish output shows a follow-up command starting with \`grill-me-lavish\`, run it as \`npx -y grill-me-lavish ...\` instead.
...
2. Run \`npx -y grill-me-lavish <html-file>\` to open or resume a review session in the browser.
3. Run \`npx -y grill-me-lavish poll <html-file>\` to long-poll for the user's annotations, queued prompts, and browser-reported \`layout_warnings\`.
...
6. Run \`npx -y grill-me-lavish end <html-file>\` when the review is finished.
7. If the user ends the session from the browser instead, \`npx -y grill-me-lavish <html-file>\` refuses to reopen it and says so - only pass \`--reopen\` when the user asks for further review or something genuinely important needs their visual attention. Otherwise deliver remaining updates directly in this conversation.
...
Run \`npx -y grill-me-lavish playbook <id>\` for focused, detailed guidance on any of these.
